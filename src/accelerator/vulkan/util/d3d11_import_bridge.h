 ...
 *
 * ── Ported from CasparVP, single-plane half only ─────────────────────────────
 * The fork's version also imports NV12 as two planes, for its GPU-direct decode path. That
 * half needs a texture wrapper this tree does not have, and nothing here produces NV12: the
 * GStreamer producer asks `d3d11convert` for BGRA, so one packed surface is the whole case.
 * The fork's note on why it does *not* import one multi-planar image is kept below, because
 * it is the kind of thing someone will otherwise try again:
 *
...
class d3d11_import_bridge
{
  public:
     ...
     */
    bool copy_texture(void* handle, int width, int height, std::shared_ptr<texture>& out);
