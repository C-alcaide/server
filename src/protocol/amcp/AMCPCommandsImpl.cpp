...
std::future<std::wstring> mixer_cdl_file_command(command_context& ctx)
{
    ...
    if (ctx.parameters.size() > 1) {
        try {
            // Probe only: whether this THROWS is the answer, so the value is deliberately
            // discarded. Cast is required because C++20's MSVC headers mark std::stoi
            // [[nodiscard]] and this tree builds with /WX.
            static_cast<void>(std::stoi(ctx.parameters.at(1)));
        } catch (...) {
