...
| **a producer's TRANSPORT parameters, or `core/producer/transport_params.h`** | `replay`, **both mixers** — it is the contract's first and so far only implementation. And note what makes its picture check able to fail at all: the fixture records colour A and then colour B **on air**, so scrubbing between them returns two DIFFERENT colours. Against this battery's older flat-colour recording, *"write `position` and the picture changes"* is **unfailable** — every frame is the same colour, and a `position` that stored a number and seeked nothing passes it. Measured: frame 25 reads (65,127,193), frame 131 reads (193,128,66), dR **+128** / dB **−127**, and R and B are exchanged between the two so a channel-order fault fails it too. **A descriptor check and a round trip pass for a dead parameter**; only a picture separates them. When P2–P4 of `PRODUCER_TRANSPORT_API_PLAN.md` land, each producer needs its own picture arm for the same reason — the contract being shared does not make one producer's implementation evidence for another's |
