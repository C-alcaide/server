...
| **the cluster module — `frame_clock`, the PTP clock, the relay, the scheduler** | **`frame_clock_self_test` at boot, and nothing else.** The frame arithmetic is a pure function and is asserted unconditionally at start-up; everything multi-node is driven by nothing. **The self-test is gated on 1001-DENOMINATOR RATES because that is where the bug was**: `ptp_ns_to_frame` computed floor(a)+floor(b) where the answer is floor(a+b), which at 25p and 50p (`den` = 1) makes the two forms IDENTICAL -- an integer-rate check passes on a broken build. It was wrong on 58% of instants at 29.97, 76% at 59.94, 78% at 23.976, always one frame low, which is a cluster disagreeing about the frame number. Its oracle is the RELATIONSHIP between `ptp_ns_to_frame` and `frame_to_ptp_ns`, not a second copy of the formula. **And the multi-node half is testable on ONE MACHINE**: `create_udp_socket` sets `SO_REUSEADDR`, so two processes can both bind the PTP ports and join the group, and `relay-port` is per-member. WSL is NOT the route -- no Linux build exists, and WSL2 is NAT'd so host-to-WSL2 UDP multicast does not work, which is exactly the PTP half |
