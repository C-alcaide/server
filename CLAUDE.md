...
| **a grading NODE, or where a node pass sits in the chain** | `grade-window` for what it computes, **`grade-graph` for WHICH SPACE it computes in** -- both mixers. And note the trap the second one exists to record: `grade-window`'s oracle is `inside == measured outside x exposure`, which is only true if the node multiplies the **already display-encoded** value. It passes at 1 LSB because that is what the prototype does, so it is a correct measurement of a placement the design says is wrong, and it **cannot see** the placement at all -- its config is a pass-through, where the two placements are provably indistinguishable. Measured 2026-09-11: with a linear middle in the chain, a node CDL and `MIXER CDL` land **42 LSB apart**, identically on both mixers |
