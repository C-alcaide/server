...
| **a producer's TRANSPORT parameters, or `core/producer/transport_params.h`** | `producer-params` for `ffmpeg` and the declaration of the GPU-direct trio, and `replay` for `replay` -- **both mixers**, plus `timeline-targets`, because a producer parameter is a timeline TARGET and `position` is the first one whose release rule differs from every other (it holds where the playhead was LEFT, not the value captured on entry). **The oracle for `position` is the BURNT-IN FRAME MARKER, not 'the picture changed'**: write `position = N` and the frame that comes back must BE N -- measured exact at 42, 137 and 7. A seek that lands one frame off produces a picture that is almost right, which is worse than one obviously wrong because it gets explained as rounding, and `frame_pin.py` records three findings and twelve cases that were exactly that. `replay`'s arm cannot use the marker (its fixture is a recording of the channel), so it records colour A then colour B ON AIR and gates on the two scrub targets returning different colours -- against its older flat-colour recording that check is UNFAILABLE, every frame being identical. **`hap` and `cuda_prores` ARE DRIVEN TO THE PICTURE NOW (2026-09-16), and the reason they were not is worth more than the coverage.** This line used to say there was 'no NotchLC or Hap encoder on this box to build a frame-identifying fixture with'. ffmpeg here has a Hap encoder and four ProRes encoders; the claim was only ever true of NotchLC, and it had parked two producers behind a blocker that did not exist. **Check whether the end you called unreachable actually is** -- the same lesson the TRACKING bullet records, and it cost the same thing twice. `marked_clip` takes a codec, and the 16-cell strip survives Hap's DXT block compression exactly (frames 0/1/7/13/19, confidence 1.0). **And the old check was reading the WRONG PRODUCER**: both `hap_producer` and `prores_producer` refuse unless their keyword is `params[0]`, so a plain `PLAY` of the fixture loaded **ffmpeg**, and the check then asserted that it declared a subset of the seven rows ffmpeg declares -- it could not fail. Every arm now asserts `foreground/producer` FIRST. `cuda_notchlc` stays declaration-only and that one is permanent: no NotchLC encoder exists outside Notch. **What driving them found immediately**: the `position` parameter stored its seek request and never notified the condition variables `CALL SEEK` notifies, so it worked on a PLAYING clip -- which is how anyone would try it -- and did nothing at all on a paused one, which is what scrubbing is. Fixed in `2637a9460`. Still open and RED on purpose: `cuda_prores` lands one frame PAST the target via `speed = 0` while exact via `PAUSE`, and all three report `position` as the DECODE clock rather than the playhead, so a write of 42 reads back 43. `gstreamer` and `artnet` are driven by nothing at all |
