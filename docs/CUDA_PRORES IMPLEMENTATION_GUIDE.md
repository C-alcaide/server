...
| 4444 alpha crash on non-16-aligned size | `ACCESS VIOLATION (0xC0000005)` ~0.8s after loading a ProRes 4444 clip whose width/height isn't a multiple of 16 (e.g. 2604×1470) | `decode_alpha_to_host` writes whole 16-row macroblock bands but `h_alpha` is allocated as exactly `width*height`; the demuxer rounds mb counts **up**, so the bottom/right edge band writes past the buffer | Pass `frame_height`; round `mb_width` up to cover the edge MB; clamp copy width and row count to the real frame dimensions; guard the alpha-payload offset against the slice size; reject `log2_slice_mb_width > 3` in the demuxer (would overflow the 2048-pixel slice buffer) |
| Grey output with random moving colored blocks | ProRes Standard (profile 2) and 4444 (profile 4) clips at 2604×1470 decode to mostly flat grey; HQ at 1920×1080 looked fine | **Slice geometry.** ProRes does not lay out a row as `ceil(mb_width/mbs_per_slice)` equal slices with one clamped remainder — when `mb_width` isn't a multiple of `mbs_per_slice`, the remainder columns are split into **power-of-two** slices (FFmpeg: `slice_mb_count` starts at `slice_mb_width` and is halved until it fits). For mb_width=163, slice=8 → 20 slices of 8, then 2 + 1 = **22 slices/row**, not `ceil=21`. Wrong slice count → all seek-table offsets drift → most slices fail header validation → grey, with the occasional aligned slice decoding → "blocks". 1920 (mb_width=120, a multiple of 8) had no remainder so it always worked | Compute `slices_per_row = (mb_width >> log2) + popcount(mb_width & (mbs_per_slice-1))` in the demuxer; replicate FFmpeg's halving walk to map slice↔MB-column in the entropy kernel, IDCT kernel (added an `mb_width` param), and `decode_alpha_to_host` |
| qscale not remapped | Higher-qscale slices dequantised ~4× too weak | Decoder used the raw slice-header qscale byte; FFmpeg remaps `qscale > 128 ? (qscale-96)<<2 : qscale` | Apply the remap in `k_prores_entropy_decode`; widen `d_q_scales` `uint8_t`→`uint16_t` (remapped value reaches 512) |
| Right/bottom edge wrong on non-16-multiple widths | Rightmost ~12px column decodes as grey when width isn't a multiple of 16 (2604) | Entropy decode was launched with `mb_width = width/16` (truncates to 162) while the seek table uses `ceil = 163`; IDCT grid used `plane_width/8` (truncates) | Use `(width+15)/16` for entropy `mb_width` at all call sites; use `(plane_width+7)/8` for the IDCT block grid (the kernel's `px < plane_width` guard keeps extra threads safe) |
