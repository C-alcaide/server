...
> **Status:** SCOPE. Nothing here is implemented. The **latency numbers in §3 are measured**
...
> parameter order changes, §2's conclusion — that no new server input path is needed — is void.
...
is a separate feature, for interactive HTML *pages*, and it is scoped separately in §6.

## 2. Interactive previz needs no new server input path — it already exists
...
at all.** The whole question is whether the resulting latency is acceptable, which is §3.
...
## 4. Do NOT go back to an embedded screen consumer
...
would re-adopt a scaling problem and a deadlock class in exchange for interactivity that §2 shows
is available over AMCP anyway. `window_embedder` should stay only as the degradation path it
currently is.

## 5. The three architectures, and the recommendation
...
**A's real defect is not duplication, it is divergence.** The client currently carries
`previz_viewport.py` (1294 lines of QOpenGLWidget with its own shaders) and
`previz_scene_model.py` (560 lines) — a complete second scene model and renderer. A previz whose
job is "show me what the server will do" is worth nothing if the two can disagree, and today
nothing forces them to agree. That is the same failure mode this tree documents everywhere else:
a second description of the truth that drifts from it.
...
## 6. Separately: reviving the CEF interaction API
...
## 7. What this scope does not establish
