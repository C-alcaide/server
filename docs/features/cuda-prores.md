...
**Owed.** The decode chain's *order* is the point (entropy → IDCT → planes → mixer shader), and it
is a case of two paths reaching the same place — planar on Vulkan 4:2:2, packed everywhere else.
Operator-facing, so a rendered PNG from `docs/diagrams/`.
